function editAbout(id, title, description, type, image) {
  const modal = document.getElementById('modal');
  document.getElementById('modal-title').innerHTML = 'Edit About';
    ...
    <div class="flex flex-row justify-between">
      <div>
        ...
        <div id="preview-container">
          <img id="image-preview" src=${image} class="w-[180px] rounded-md mt-2" />
        </div>
      </div>
      ...
      <div class="ml-3 flex flex-col w-[350px]">
        ...
        <div class="mb-4">
          ...
          <select id="about-type" name="type" class="w-full px-3 py-2 mt-1 border rounded-md">
            <option value="Q_AND_A" ${type === 'Q_AND_A' ? 'selected' : ''}>Q_AND_A</option>
...
async function confirmUpdateAbout(id) {
  ...
  if (result.errors) {
    showNotification('Cannot update About, something went wrong!', 'fail');
  } else {
    alert('Success! You have successfully updated About!');
    closeModal();
