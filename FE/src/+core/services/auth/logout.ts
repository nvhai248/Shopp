import { LogoutMutation } from "@/+core/definegql";
import { MyGraphqlResponse } from "@/+core/types/response";
import { MyApolloClient } from "@/lib/apolloClient";
import { GraphQLError } from "graphql";

export async function Logout(refreshToken: string): Promise<MyGraphqlResponse> {
  try {
    ...
    return {
      data: null,
      errors: [
        new GraphQLError(error.message || "An error occurred during logout."),
      ],
    };
  }
}
